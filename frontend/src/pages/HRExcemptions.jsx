...
function HRExcemptions() {
    ...
    const fetchExemptions = async () => {
        try {
            const res = await axios.get("/attendance/hr_exemptions_all");
    ...
    useEffect(() => {
        const fetchStaffName = async () => {
            if (formData.staffId && formData.staffId.length > 2) {
                try {
                    const res = await axios.post('/attendance/search/getuser', { staffId: formData.staffId });
    ...
    const handleSubmit = async (e) => {
        e.preventDefault();
        try {
            const res = await axios.post("/attendance/applyExemption", {
                ...formData,
    ...
    const downloadPDF = () => {
        ...
        PdfTemplate({
            title,
            tables: [{ columns, data }],
            fileName: `Exemptions (${startDate} - ${endDate}).pdf`,
            fromDate: startDate,
            toDate: endDate
        });
    };
